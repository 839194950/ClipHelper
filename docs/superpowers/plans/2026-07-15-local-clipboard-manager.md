...
**Tech Stack:** C# 14、.NET 10 LTS、WinForms、Microsoft.Data.Sqlite、xUnit、Windows Clipboard API、RegisterHotKey、Named Mutex、Named Pipe
...
Run:
    ...
    dotnet add src/LocalClipboard.Infrastructure/LocalClipboard.Infrastructure.csproj package Microsoft.Data.Sqlite --version 10.0.0

Expected: restore succeeds with no package downgrade warning.
...
2. Create AppPaths and RollingFileLogger.
3. Build the per-user SingleInstanceCoordinator name from WindowsIdentity.GetCurrent().User.Value.
4. If TryAcquirePrimary is false, call SendShowMessageAsync and return.
5. Load settings; if StartWithWindows is true, refresh the current-user startup path.
6. Open SQLite repository. Treat SQLite primary error codes 11 (CORRUPT) and 26 (NOTADB) as confirmed corruption, move database sidecar files into recovery, and retry once.
7. Create PngImageStore and delete orphan files using paths referenced by repository.GetAllAsync.
8. Create HistoryService and run EnforceRetentionAsync(UtcNow).
9. Create TrayApplicationContext and start a named-pipe listener loop that calls ShowPopup for each show message.
10. Call Application.Run(context).
